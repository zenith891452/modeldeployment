...
# Muat model dan encoder setelah file di-upload
if model_file is not None and encoder_file is not None:
    # Membaca file model dan encoder menggunakan BytesIO
    model_data = BytesIO(model_file.read())
    encoder_data = BytesIO(encoder_file.read())

    # Muat model dan encoder dari file
    model = pickle.load(model_data)
    label_encoders = pickle.load(encoder_data)
